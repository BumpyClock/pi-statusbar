...
- **User-defined presets** — Define layouts in `statusbar.presets` with `extends` inheritance, explicit `custom:<id>` segment placement, and settings merge/override semantics; use `/statusbar <preset>` to apply them. See README for full configuration.

### Changed

- **Fixed-editor defaults** — Fixed editor and mouse-scroll handling are documented as enabled by default; set `statusbar.fixedEditor: false` or `statusbar.mouseScroll: false` to opt out.
