...
// Uses module-level state (matching pi-statusbar pattern).
...
async function generateVibe(
	...
	if (
		...
		console.debug(
			`[working-vibes] Vibe generation failed for ${config.modelSpec}: ${response.errorMessage}`,
		);
	}
...
export async function generateVibesBatch(
	...
	if (!model) {
		return {
			...
			count: 0,
			filePath,
			error: `Model not found: ${config.modelSpec}`,
		};
	}
	...
	if (!auth.ok) {
		return {
			...
			count: 0,
			filePath,
			error: "error" in auth ? auth.error : "unknown auth error",
		};
	}
