...
test("terminal split keeps tabbed overlay composition within terminal width", () => {
	...
	const overlay =
		'\x1b[38;2;119;125;136m[grep]: render.ts-706- \treturn [...lines.slice(0, visibleLines), truncLine(theme.fg("dim", hint), width)];\x1b[39m';

	const before = (tui as any).compositeLineAt(
		"Validation before " + " ".repeat(232),
		overlay,
		20,
		210,
		250,
	...
	compositor.install();
	const after = (tui as any).compositeLineAt(
		"Validation before " + " ".repeat(232),
		overlay,
		20,
		210,
...
test("terminal split handles modified SGR wheel packets", () => {
	...
	assert.deepEqual(inputListener?.("\x1b[<68;1;1M\x1b[<68;1;1M"), {
		consume: true,
	});
	assert.deepEqual(renderRequests, [undefined, undefined]);
	assert.deepEqual(tui.render(40), [
...
test("terminal split pauses mouse reporting on right click for the terminal context menu", () => {
	...
	assert.ok(
		terminal.writes.at(-1)?.includes("\x1b[?1006l\x1b[?1002l\x1b[?1000l"),
	);
	assert.deepEqual(inputListener?.("\x1b[<0;5;5M"), { consume: true });
	assert.deepEqual(inputListener?.("\x1b[<0;5;5m"), { consume: true });
...
test("terminal split selection highlighting does not duplicate wide glyphs", () => {
	for (const glyph of ["🪃", "👨‍👩‍👧‍👦"]) {
		...
		assert.deepEqual(inputListener?.(`\x1b[<0;${glyphCol + 1};10M`), {
			consume: true,
		});
		assert.deepEqual(inputListener?.(`\x1b[<32;${glyphCol + 2};10M`), {
			consume: true,
		});
