...
export default function statusbar(pi: ExtensionAPI) {
	...
	config = parseStatusbarConfig(startupSettings.statusbar, PRESET_NAMES);
	...
	// Cache for the top and secondary statusbar widgets.
	...
	function getStatusbarShortcutAction(
		data: string,
	): StatusbarShortcutAction | null {
	...
	function runStatusbarShortcut(
		ctx: any,
		action: StatusbarShortcutAction,
	): void {
		...
		handler: async (args, ctx) => {
			...
			if (!args?.trim()) {
				...
				if (enabled) {
					...
					ctx.ui.setHeader(undefined);
					ctx.ui.setWidget("statusbar-top", undefined);
					ctx.ui.setWidget("statusbar-secondary", undefined);
					ctx.ui.setWidget("statusbar-bash-transcript", undefined);
					ctx.ui.setWidget("statusbar-status", undefined);
	...
	pi.registerCommand("stash-history", {
		...
		handler: async (_args, ctx) => {
			if (!ctx.hasUI) return;
			if (!enabled) {
				ctx.ui.notify("Statusbar is disabled", "info");
				return;
			}
	...
	pi.registerShortcut(bashModeSettings.toggleShortcut as ShortcutKeyId, {
	...
	pi.registerShortcut("alt+s" as ShortcutKeyId, {
	...
	pi.registerShortcut(resolvedShortcuts.stashHistory as ShortcutKeyId, {
		description: "Open prompt history picker",
		handler: async (ctx) => {
			if (!enabled || !ctx.hasUI) return;
			await openStashHistory(ctx);
	...
	pi.registerShortcut(resolvedShortcuts.copyEditor as ShortcutKeyId, {
	...
	pi.registerShortcut(resolvedShortcuts.cutEditor as ShortcutKeyId, {
	...
	for (const { shortcutKey, description, action } of CHAT_JUMP_SHORTCUTS) {
		pi.registerShortcut(resolvedShortcuts[shortcutKey] as ShortcutKeyId, {
			description,
			handler: async (ctx) => {
				if (!enabled || !ctx.hasUI) return;
				runStatusbarShortcut(ctx, { kind: "chat", action });
			},
		});
	}
	...
	function renderStatusbarStatusLines(width: number): string[] {
	...
	function renderStatusbarTopLines(width: number, theme: Theme): string[] {
	...
	function renderStatusbarSecondaryLines(
		width: number,
		theme: Theme,
	): string[] {
		if (!currentCtx) return [];
	...
	function installFixedEditorCompositor(ctx: any, tui: any) {
		...
		if (!tui?.terminal || typeof tui.terminal.write !== "function") {
			throw new Error(
				"[pi-statusbar] Fixed editor compositor could not find tui.terminal.write()",
			);
		}
		if (!currentEditor) {
			throw new Error(
				"[pi-statusbar] Fixed editor compositor expected the custom editor to be installed first",
			);
		}
		...
		if (!editorContainerMatch) {
			throw new Error(
				"[pi-statusbar] Fixed editor compositor could not find the editor container in TUI children",
			);
		}
		...
		compositor = new TerminalSplitCompositor({
			...
			renderCluster: (width, terminalRows) => {
				...
				return renderFixedEditorCluster({
					...
					statusLines: [
						...aboveWidgetLines,
						...renderStatusbarStatusLines(width),
						...statusContainerLines,
					],
					topLines: renderStatusbarTopLines(width, theme),
					editorLines: fixedEditorContainer
						? compositor.renderHidden(fixedEditorContainer, width)
						: [],
					secondaryLines: [
						...renderStatusbarSecondaryLines(width, theme),
						...belowWidgetLines,
					],
	...
	function jumpToChatMessage(
		...
		if (!fixedEditorCompositor) {
			ctx.ui.notify(
				"Chat message jumps require /statusbar fixed-editor on",
				"warning",
			);
			return;
		}
	...
	function jumpChatToBottom(ctx: any): void {
		if (!fixedEditorCompositor) {
			ctx.ui.notify(
				"Chat bottom jump requires /statusbar fixed-editor on",
				"warning",
			);
			return;
		}
	...
	function installStatusbarWidgets(ctx: any) {
		ctx.ui.setWidget(
			"statusbar-status",
			() => ({
				dispose() {},
				...
				render(width: number): string[] {
					return renderStatusbarStatusLines(width);
				},
			}),
		...
		ctx.ui.setWidget(
			...
			(_tui: any, theme: Theme) => ({
				...
				render(width: number): string[] {
					return renderStatusbarTopLines(width, theme);
				},
			}),
			{ placement: "aboveEditor" },
		);
		...
		ctx.ui.setWidget(
			...
			(_tui: any, theme: Theme) => ({
				...
				render(width: number): string[] {
					return renderStatusbarSecondaryLines(width, theme);
				},
			}),
		...
		stashShortcutInputUnsubscribe =
			typeof ctx.ui.onTerminalInput === "function"
				? ctx.ui.onTerminalInput((data: string) => {
						...
						const statusbarShortcutAction = getStatusbarShortcutAction(data);
						if (!statusbarShortcutAction) {
							return undefined;
						}

						runStatusbarShortcut(ctx, statusbarShortcutAction);
						scheduleDismissWelcome(ctx);
						tuiRef?.requestRender();
						return { consume: true };
					})
			...
			const attachAutocompleteProvider = (): boolean => {
				...
				editor.installAutocompleteProvider(
					new ModeAwareAutocompleteProvider(
						defaultProvider,
						bashProvider as unknown as AutocompleteProvider,
						oneOffBashProvider as unknown as AutocompleteProvider,
						() => bashModeActive,
	...
	function setupWelcomeOverlay(ctx: any) {
		...
		setTimeout(() => {
			...
			ctx.ui
				...
				.catch((error: unknown) => {
					console.debug("[pi-statusbar] Welcome overlay failed:", error);
				});
		}, 100);
	}
}
